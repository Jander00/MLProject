...
Predicting the model set based on this model gives `r 100*signif(mean(Y_train!=Y_train_pred),3)`% error. So it perferctly predicts the training values of the response variable.
...
This gives an error of `r 100*signif(mean(Y_cv!=Y_cv_pred),3)`%, so clearly this seems like a very strong model. However it is worth playing with some elements to see if we can improve the accuracy.
...
Looking at the errors, we see `r 100*signif(mean(Y_train!=Y_train_pred),3)`% for the training data and `r 100*signif(mean(Y_cv!=Y_cv_pred),3)`% for the cross validation data.
...
Running this code, the accuracy we get on this dataset is `r 100*signif(mean(Y_test!=Y_test_pred),3)`%.
